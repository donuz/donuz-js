...
export var prizes= {
    ...
    {
        return get('premio/'+config.estabelecimento_id, {'Token':config.token, 'Order':order, 'Limit':limit});
    },getRewardByBranch:function(idFilial, order = null, limit = null)
    {
        ...
        return post('premio', {'Token':config.token, 'Order':order, 'Limit':limit}, data);
    ...
    {
        return get('premio/'+config.estabelecimento_id + '/'+idPremio, {'Token':config.token, 'Order':order, 'Limit':limit});
